...
def assemble_prompt(parts: ChallengeParts) -> str:
    ...
    return (
        ...
        'The only thing I write is the solution function. The '
        '`@mark.parametrize` decorator, the test function signature, `tests`, and the '
        "`if __name__ == '__main__'` block are fixed and kept identical in structure "
        'across all challenge files; `__main__` is debugging scaffolding used while '
        'developing, not part of the evaluation.\n'
        'Evaluate the solution the way a technical interviewer or senior engineer '
        'doing code review would assess a candidate for a junior/mid-level dev or '
        'software engineering role. Consider: correctness against the given test '
        'cases and against the problem description more broadly, code clarity, and '
        'whether unspecified or edge-case behavior implied by the description (e.g. '
        'tie-breaks, invalid input) was handled deliberately or left to chance. Give '
        'a hiring-relevant assessment (e.g. "junior-ready", "needs more rigor for '
        'mid-level", "solid but undertested") as the first line, with a short '
        'justification. Note any strengths that would stand out for a '
        'career-transitioner without inflating the assessment itself. Calibrate '
        'against realistic professional practice, not idealized exhaustive coverage.\n'
